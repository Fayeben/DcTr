...
class addgaussiannoise(object):
    ...
    def __call__(self, ptcloud):
        mu, sigma = 0, 0.01
        len = (ptcloud.shape[0]) // 10
        ptcloud_shuffle = ptcloud
        np.random.shuffle(ptcloud_shuffle)
        noise = ptcloud_shuffle[:len, :]
        noisy_pcd = noise
        noisy_pcd = np.random.normal(mu, sigma, size=noisy_pcd.shape) + noise
        #source_noisy = apply_noise(noise, mu, sigma)
        # print(source_noisy)
        pcwithnoise = np.concatenate((ptcloud, noisy_pcd), axis=0)
        return pcwithnoise
